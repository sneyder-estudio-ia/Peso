...
const getRecordsForDate = (dateString) => {
    ...
    const isRecurringOnDate = (recurrence, checkDayOfMonth, checkDayOfWeek) => {
...
const openDayDetailsModal = async (dateString) => {
    ...
    const createRecordListSection = (title, records, type) => {
        ...
        if (records.length === 0) {
            ...
            records.forEach(record => {
                ...
                if (type === 'income')
                    subText += ` • Fuente: ${record.source || 'N/A'}`;
                if (type === 'expense')
                    subText += ` • Cat: ${record.category || 'General'}`;
                if (type === 'savings' && record.goalAmount)
                    subText += ` • Meta: ${formatCurrency(record.goalAmount, { includeSymbol: true })}`;
        ...
        const populateRecordsByDate = () => {
            ...
            const processRecords = (records, type) => {
                records.forEach(record => {
                    ...
                    const itemsToCheck = (type === 'expense' && record.isGroup)
                        ? record.items || []
                        : [record];
                    itemsToCheck.forEach(item => {
                        ...
                        if (record.type === 'Único' && item.date) {
                            ...
                            if (recYear === year && recMonth === month + 1) {
                                if (!recordsByDate[item.date])
                                    recordsByDate[item.date] = { income: false, expense: false, savings: false };
                            ...
                            for (let day = 1; day <= daysInMonth; day++) {
                                ...
                                if (eventHappens) {
                                    ...
                                    if (!recordsByDate[dateString])
                                        recordsByDate[dateString] = { income: false, expense: false, savings: false };
