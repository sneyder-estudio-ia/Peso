...
export const renderSettingsView = (container: HTMLElement, navigate: NavigateFunction) => {
    ...
    geminiSection.className = 'stats-section';
    const geminiCard = createSimpleCard('Inteligencia Artificial (Gemini)');

    const geminiForm = document.createElement('form');
    geminiForm.className = 'income-form';
    geminiForm.style.marginTop = '15px';
    geminiForm.onsubmit = (e) => e.preventDefault();

    const apiKeyField = createFormField('Tu API Key de Gemini', 'password', 'user-gemini-apikey', 'geminiApiKey');
    
    const saveApiKeyButton = document.createElement('button');
    saveApiKeyButton.textContent = 'Guardar API Key';
    saveApiKeyButton.className = 'btn btn-add';
    saveApiKeyButton.style.width = '100%';
    saveApiKeyButton.style.marginTop = '20px';
    saveApiKeyButton.onclick = () => {
        const apiKey = (geminiForm.querySelector('#user-gemini-apikey') as HTMLInputElement).value;
        appState.userProfile.geminiApiKey = apiKey;
        saveState(appState);
        showToast('API Key de Gemini guardada.');
    };

    geminiForm.appendChild(apiKeyField);
    geminiForm.appendChild(saveApiKeyButton);
    geminiCard.appendChild(geminiForm);
    geminiSection.appendChild(geminiCard);
    ...
    if (currentUserProfile) {
        ...
        (apiKeyField.querySelector('input') as HTMLInputElement).value = currentUserProfile.geminiApiKey || '';
    }
