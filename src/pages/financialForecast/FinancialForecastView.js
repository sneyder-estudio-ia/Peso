...
const getDetailedTransactionsForMonth = (year, month) => {
            ...
            for (let day = 1; day <= daysInMonth; day++) {
                ...
                switch (record.recurrence.type) {
                    case 'Diario':
                        ...
                        break;
                    case 'Semanal':
                        eventHappens = checkDate.getDay() === dayNameToIndex[record.recurrence.dayOfWeek];
                        break;
                    case 'Mensual':
                    case 'Quincenal':
                        eventHappens = record.recurrence.daysOfMonth?.includes(day) ?? false;
                        break;
                }
                if (eventHappens) {
    ...
    appState.expenseRecords.forEach(record => {
        ...
        const itemsToCheck = record.isGroup ? record.items || [] : [record];
                    ...
                    for (let day = 1; day <= daysInMonth; day++) {
                        ...
                        switch (item.recurrence.type) {
                            case 'Diario':
                                ...
                                break;
                            case 'Semanal':
                                eventHappens = checkDate.getDay() === dayNameToIndex[item.recurrence.dayOfWeek];
                                break;
                            case 'Mensual':
                            case 'Quincenal':
                                eventHappens = item.recurrence.daysOfMonth?.includes(day) ?? false;
                                break;
                        }
                        if (eventHappens) {
...
const createCashflowBarChart = (income, expense, savings) => {
    ...
    const createBar = (label, value, type) => {
...
export const renderFinancialForecastView = (container, navigate) => {
    ...
    const formatComparison = (current, previous) => {
        if (previous === 0)
            return `<div class="summary-comparison">vs. ${formatCurrency(0, { includeSymbol: true })}</div>`;
