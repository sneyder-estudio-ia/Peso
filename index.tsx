...
async function main() {
    ...
    const archiveOldUniqueRecords = async () => {
        ...
        const expenseRecordsToArchive: ExpenseRecord[] = [];
        const remainingExpenseRecords: ExpenseRecord[] = [];
        
        const incomeRecordsToArchive: IncomeRecord[] = [];
        const remainingIncomeRecords: IncomeRecord[] = [];
        ...
        const getPayDaysForMonth = (year: number, month: number): number[] => {
            ...
            salaries.forEach(salary => {
                ...
                switch (recurrence.type) {
                    case 'Semanal':
                        if (recurrence.dayOfWeek) {
                            ...
                            if (targetDayIndex !== undefined) {
                                for (let day = 1; day <= monthDays; day++) {
                                    const date = new Date(year, month, day);
                                    if (date.getDay() === targetDayIndex) payDays.push(day);
                                }
                            }
                        }
                        break;
                    case 'Quincenal': case 'Mensual':
                        if (recurrence.daysOfMonth) {
                            recurrence.daysOfMonth.forEach(day => { if (day > 0 && day <= monthDays) payDays.push(day); });
                        }
                        break;
                    case 'Diario': default: break;
                }
        ...
        const getPeriodForDate = (dateToCheck: Date): { start: Date, end: Date } | null => {
        ...
        for (const record of appState.expenseRecords) {
            if (record.type === 'Único' && !record.isGroup && record.date) {
                ...
                const remainingItems: ExpenseSubItem[] = [];
